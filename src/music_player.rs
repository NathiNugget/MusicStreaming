...
use crate::song_libery::print_songs;
use crate::user_input::read_number;
use std::sync::{Arc, Mutex}; // til brug at threading // TOdO

pub fn play_song(songs: &Vec<String>) ->  Result<(), Error> {
    if songs.len() == 0 {
    ...
    pub fn add_to_sink(idx :u16, songs :&Vec<String>) -> Result<Option<Decoder<BufReader<File>>>, Error> {
